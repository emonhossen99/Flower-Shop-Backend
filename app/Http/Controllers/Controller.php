...
class Controller extends BaseController
{
     ...
     * @return \Illuminate\Http\Response
     */
    protected function imageUploadWithCrop($file, $folderName, $width, $height)
    {
        if ($file) {
            $folder = 'uploads/' . $folderName;
            ...
            if ($width != null && $height != null) { // width and height null value check
                Image::make($file)->resize($width, $height)->save($folder . $imageName); // resize imageName
                $path = $folder . $imageName;
            } else {
                $file->move($folder, $imageName); // not resize imageName
                $path = $folder . $imageName;
            }
        } else {
            $path = null;
