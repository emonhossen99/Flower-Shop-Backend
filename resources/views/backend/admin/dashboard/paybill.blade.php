...
@section('content')
        ...
        <div class="mobile-container">
            <div>
                <div class="header">
                    ...
                    @endphp
                    <div class="profile-circle {{ $role == 'customer' ? 'customer' : 'supplier' }}">{{ $username ?? '' }}
                    </div>
                    <div class="contact-info ms-2">
                        <h5>{{ $role == 'customer' ? $user->customer_name ?? '' : $user->name }}</h5>
                        <small><i class="fa-solid fa-phone me-1"></i>{{ $user->phone ?? '' }}</small>
                    </div>
                    <div class="ms-auto">
                        <div class="btn-group dropstart">
                            <button type="button" class="btn dropdown-toggle border-0 bg-transparent"
                ...
                <div class="balance-section">
                    <div>
                        @php
                            $totaldue = ($user->previous_due ?? 0) + ($user->lastpayment->due ?? 0);
                        @endphp
                        @if ($role == 'customer')
                            @if ($totaldue >= 0)
                                <span>পাবো</span><span class="balance-amount negetive"> {{ currency() }}
                                    {{ str_replace('-', '', convertToLocaleNumber(($user->previous_due ?? 0) + ($user->lastpayment->due ?? 0))) }}</span>
                            @else
                                <span>দেবো</span><span class="balance-amount positive"> {{ currency() }}
                                    {{ str_replace('-', '', convertToLocaleNumber(($user->previous_due ?? 0) + ($user->lastpayment->due ?? 0))) }}</span>
                            @endif
                        @else
                            @if ($totaldue >= 0)
                    ...
                    <span
                        class="balance-label">({{ \Carbon\Carbon::parse($user->lastpayment->created_at ?? $user->created_at)->diffForHumans() }})</span>
                </div>
                <form id="customerSupplierPaybillFrom"
                    action="{{ route('admin.dashboard.pay.bill.customer.supplier.store') }}" method="POST"
                        ...
                        <div class="row">
                            <div class="col-6 col-md-6">
                                <div class="input-group mb-3">
                                    ...
                                    <input type="number" id="total_amount" name="total_amount" class="form-control px-0"
                                        placeholder="{{ $role == 'customer' ? 'দিলাম/বেচা' : 'দিলাম' }}"
                            ...
                            <div class="col-6 col-md-6">
                                <div class="input-group mb-3">
                                    <span class="input-group-text px-2 py-1" id="basic-addon1"><span class="fs-4"
                                            style="color: #666;">৳</span></span>
                                    <input type="number" id="pay_amount" name="pay_amount" class="form-control px-0"
                                        placeholder="{{ $role == 'customer' ? 'পেলাম' : 'পেলাম/কেনা' }}"
                                        aria-describedby="basic-addon1" autocomplete="off">
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="nav-bottom">
                        <div class="nav-item">
                            ...
                            <span id="customDateTrigger">{{ formatDateToBangla(now()) }}</span>
                            <input type="date" id="paydate" name="pay_date" value="{{ now()->format('Y-m-d') }}"
                                class="d-none">
                        </div>
                        <span style="display: flex; align-items: center;">
                            <div class="form-check form-switch">
                                <label class="form-check-label" for="message_status">মেসেজ</label>
                                <input class="form-check-input" name="message" type="checkbox" id="message_status"
                                    value="1">
                            </div>
                        </span>
                        <div class="nav-item">
    ...
    @include('backend.admin.dashboard.uppercashmodal')
@endsection
@push('scripts')
    <script>
        ...
        $('#paydate').on('change', function() {
            ...
            let banglaDate = convertToBanglaDate(selectedDate);
